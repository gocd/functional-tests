...
def task_plugin
  ...
  sh "wget --silent #{url} -O target/go-server-#{VERSION_NUMBER}/plugins/external/task-skeleton-plugin.jar"
end

task :setup_tfs_cli do
    mkdir_p "tfs-tool"
    sh "wget --silent https://maven-mirrors.gocd.org/repository/s3-mirrors/local/TEE-CLC-14.0.3.zip -O tfs-tool/TEE-CLC-14.0.3.zip"
