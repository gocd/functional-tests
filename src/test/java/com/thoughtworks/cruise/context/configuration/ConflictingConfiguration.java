...
public class ConflictingConfiguration extends AbstractConfiguration {

	private Configuration config;

	public ConflictingConfiguration(Configuration config, ScenarioState state, RepositoryState repositoryState, ScenarioHelper scenarioHelper, Browser browser) throws Exception {
		super("/config/conflict-cruise-config.xml", config, state, repositoryState, scenarioHelper, browser);
		this.config = config;
	}

	@com.thoughtworks.gauge.Step("ConflictingConfiguration - setup")
	@Override
	...
	@com.thoughtworks.gauge.Step("ConflictingConfiguration - teardown")
	@Override
