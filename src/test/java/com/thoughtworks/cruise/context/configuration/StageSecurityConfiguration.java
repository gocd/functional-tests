...
public class StageSecurityConfiguration extends AbstractConfiguration {

	private Configuration config;

	public StageSecurityConfiguration(Configuration config, ScenarioState state, RepositoryState repositoryState, ScenarioHelper scenarioHelper, Browser browser) throws Exception {
		super("/config/stage-security-cruise-config.xml", config, state, repositoryState, scenarioHelper, browser);
		this.config = config;
	}

	@com.thoughtworks.gauge.Step("Stage security configuration - setup")
	@Override
	...
	@com.thoughtworks.gauge.Step("Stage security configuration - teardown")
	@Override
